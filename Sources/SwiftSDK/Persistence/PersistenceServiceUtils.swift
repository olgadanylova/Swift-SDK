...
class PersistenceServiceUtils {
    ...
    private func findOffline(queryBuilder: DataQueryBuilder?, responseHandler: (([[String : Any]]) -> Void)!, errorHandler: ((Fault) -> Void)!) {
        ...
        let result = localManager.select(withDeleted: false, properties: queryBuilder?.properties, whereClause: queryBuilder?.whereClause, limit: nil, offset: nil, orderBy: queryBuilder?.sortBy, groupBy: queryBuilder?.groupBy, having: queryBuilder?.havingClause)
    ...
    private func findFirstOrLastOrByIdOffline(first: Bool, last: Bool, objectId: String?, queryBuilder: DataQueryBuilder?, responseHandler: (([String : Any]) -> Void)!, errorHandler: ((Fault) -> Void)!) {
        ...
        if first || last {
            ...
            let result = localManager.select(withDeleted: false, properties: queryBuilder?.properties, whereClause: whereClause, limit: nil, offset: nil, orderBy: queryBuilder?.sortBy, groupBy: queryBuilder?.groupBy, having: queryBuilder?.havingClause)
